...
impl Client {
    ...
    async fn list_issue_comments(&self, repo: &str, number: u64) -> Result<serde_json::Value> {
        let path = format!("/repos/{repo}/issues/{number}/comments?per_page=100");
        self.get(&path).await
    }

    async fn search_code(&self, query: &str, limit: u32, page: u32) -> Result<serde_json::Value> {
...
fn print_issue_comments(value: &serde_json::Value) {
        ...
        Commands::Issue { command } => {
            ...
            match command {
                ...
                IssueCommands::Comments { repo, number } => {
                    let result = client.list_issue_comments(&repo, number).await?;
                ...
                PrCommands::Review { repo, number, body, event, comments } => {
                    let inline_comments: Vec<serde_json::Value> = comments
                        ...
                        .map(|c| {
                            ...
                            // Interpret escape sequences in comment body
                    ...
                    // Interpret escape sequences in body too
